...
class ErgoCubBimanualController:
    ...
    def __init__(self, remote_prefix: str, local_prefix: str, use_left_hand: bool = True, use_right_hand: bool = True, use_torso: bool = True):
        ...
        # Regularize joint velocities to reduce jitter.
        self.damping_task = DampingTask(cost=1e-2)
        ...
        self._driver = None
        self._ipos = None
        # Use Position mode interfaces
        self._icmd = None
        self._ienc = None

        # Position control reference speed (deg/s) used by send_command/reset.
        # Tune this at runtime, e.g. `controller.position_velocity_deg_s = 15.0`.
        self.position_velocity_deg_s = 30.0
    ...
    def connect(self) -> None:
        ...
        for j in range(len(self.joint_names)):
            self._icmd.setControlMode(j, yarp.VOCAB_CM_POSITION)
    ...
    def read_current_state(self) -> Dict[str, float]:
        ...
        for side in ["left", "right"]:
            ...
            if side in self.kinematics_solvers:
                ...
                rot6d = matrix_to_rotation_6d(torch.tensor(pose_matrix[:3, :3]))
                ...
                state.update({
                    ...
                    f"{side}_hand.orientation.d1": rot6d[0].item(),
                    f"{side}_hand.orientation.d2": rot6d[1].item(),
                    f"{side}_hand.orientation.d3": rot6d[2].item(),
                    f"{side}_hand.orientation.d4": rot6d[3].item(),
                    f"{side}_hand.orientation.d5": rot6d[4].item(),
                    f"{side}_hand.orientation.d6": rot6d[5].item(),
                })
    ...
    def send_command(self, q: np.ndarray = None) -> None:
        """Send target joints through Position control.
    ...
    def send_commands(self, commands: dict[str, float]) -> None:
        ...
        for hand_name, hand_cmds in [("left", left_hand_cmds), ("right", right_hand_cmds)]:
            if hand_cmds:
                rot_matrix = rotation_6d_to_matrix(torch.tensor(list(hand_cmds.values())[3:9], dtype=torch.float32)).numpy()
                pos = np.array([hand_cmds[key] for key in ["position.x", "position.y", "position.z"]])

                self.tasks[hand_name].set_target(
                            pin.SE3(rot_matrix, pos)
                        )
        ...
        for _ in np.arange(5):
            velocity = solve_ik(
                configuration,
                [self.tasks["left"], self.tasks["right"], self.posture_task, self.damping_task],
                dt,
                solver="proxqp",
                safety_break=False,
            )
