...
def make_robot_action(action_tensor: PolicyAction, ds_features: dict[str, dict]) -> RobotAction:
    ...
    tensor_dim = int(action_tensor.shape[0])
    n = min(len(action_names), tensor_dim)

    act_processed_policy: RobotAction = {
        f"{name}": float(action_tensor[i]) for i, name in enumerate(action_names[:n])
    }

    # Keep dataset recording robust when schema has extra named actions.
    # Missing names default to 0.0.
    for name in action_names[n:]:
        act_processed_policy[name] = 0.0

    # Backward-compatible mapping for models that output one extra channel
    # beyond declared action names (used by some emotion-enabled checkpoints).
    if tensor_dim > len(action_names) and "emotions" not in act_processed_policy:
        act_processed_policy["emotions"] = float(action_tensor[len(action_names)])
